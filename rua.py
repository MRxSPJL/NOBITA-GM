...
def main():
    os.system('clear')
    logo();os.system('xdg-open https://t.me/DARK_TEAM_LMNx9')
    try:limit = int(input('\n\033[1;32m ENTER UA LIMIT : '))
    except ValueError:
        lxx()
    ...
    for x in range(limit):
        ...
        open('/sdcard/LMNx9-UA.txt', 'a').write(''+user_agent+'\n\n')
    lnx()
    print(f'\n{colour}</>{colour} ALL UA SAVED IN{colour} - {colour}LMNx9-UA.txt')
    lxx()
    lj=input(f'{colour}</> RUN AGAIN ? [Y/n] : ')
    if lj in ('y','Y'):
        main()
    elif lj in ('n','N'):
        lxx()
        sys.exit(f'{colour}</> Thanks For Using LMNx9 Tool ')
    else:
        lxx()
        sys.exit()
